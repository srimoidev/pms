...
router.get("/", (req, res) => {
  ...
  if (Object.entries(data).length == 0) {
    database.query("SELECT * FROM FormStatus", (err, rows) => {
    ...
    database.query(
      `SELECT * FROM FormStatus WHERE ${condition}`,
      (err, rows) => {
        if (err) res.status(400).send({ success: false, data: err.sqlMessage });
        else {
          if (rows.length == 0)
...
router.get("/:id", (req, res) => {
  database.query(
    "SELECT * FROM FormStatus WHERE FormStatus_ID = ?",
    [req.params.id],
    (err, rows) => {
      if (err) res.status(400).send({ success: false, data: err.sqlMessage });
      else {
...
router.post("/", (req, res) => {
  const data = {
    FormStatus_Name: req.body.FormStatus_Name,
  };
  database.query("INSERT INTO FormStatus SET ?", data, (err) => {
    if (err) res.status(400).send({ success: false, data: err.sqlMessage });
    else
      res
        .status(200)
  ...
  database.query(
    ...
    (err, rows) => {
      if (err) res.status(400).send({ success: false, data: err.sqlMessage });
        ...
        database.query(
          "UPDATE FormStatus SET ? WHERE FormStatus_ID = ?",
  ...
  database.query(
    ...
    (err, rows) => {
      if (err) res.status(400).send({ success: false, data: err.sqlMessage });
        ...
        database.query(
          "DELETE FROM FormStatus WHERE FormStatus_ID = ?",
          req.params.id,
          (err) => {
            if (err)
              res.status(400).send({ success: false, data: err.sqlMessage });
