...
router.get("/", (req, res) => {
  ...
  if (Object.entries(data).length == 0) {
    database.query("SELECT * FROM FormType", (err, rows) => {
    ...
    database.query(
      `SELECT * FROM FormType WHERE ${condition}`,
      (err, rows) => {
        if (err) res.status(400).send({ success: false, data: err.sqlMessage });
        else {
          if (rows.length == 0)
...
router.get("/:id", (req, res) => {
  database.query(
    "SELECT * FROM FormType WHERE FormType_ID = ?",
    [req.params.id],
    (err, rows) => {
      if (err) res.status(400).send({ success: false, data: err.sqlMessage });
      else {
...
router.post("/", (req, res) => {
  const data = {
    FormType_Name: req.body.FormType_Name,
  };
  database.query("INSERT INTO FormType SET ?", data, (err) => {
    if (err) res.status(400).send({ success: false, data: err.sqlMessage });
    else
      res
        .status(200)
  ...
  database.query(
    ...
    (err, rows) => {
      if (err) res.status(400).send({ success: false, data: err.sqlMessage });
        ...
        database.query(
          "UPDATE FormType SET ? WHERE FormType_ID = ?",
  ...
  database.query(
    ...
    (err, rows) => {
      if (err) res.status(400).send({ success: false, data: err.sqlMessage });
        ...
        database.query(
          "DELETE FROM FormType WHERE FormType_ID = ?",
          req.params.id,
          (err) => {
            if (err)
              res.status(400).send({ success: false, data: err.sqlMessage });
