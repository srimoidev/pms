...
router.get("/", (req, res) => {
  ...
  if (Object.entries(data).length == 0) {
    database.query(
      "SELECT * FROM NotificationTeacher AS N " +
        "LEFT JOIN UserTeacher AS T ON N.Notification_TeacherID=T.Teacher_ID",
      (err, rows) => {
...
router.post("/", (req, res) => {
  const data = {
    Notification_TeacherID: req.body.Notification_TeacherID,
  ...
  database.query("INSERT INTO NotificationTeacher SET ?", data, (err) => {
    if (err) res.status(400).send({ success: false, data: err.sqlMessage });
...
router.put("/:id", (req, res) => {
  database.query(
    "SELECT * FROM NotificationTeacher WHERE Notification_ID = ?",
    req.params.id,
    (err, rows) => {
      if (err) res.status(400).send({ success: false, data: err.sqlMessage });
        ...
        database.query(
          "UPDATE NotificationTeacher SET ? WHERE Notification_ID = ?",
  ...
  database.query(
    ...
    (err, rows) => {
      if (err) res.status(400).send({ success: false, data: err.sqlMessage });
        ...
        database.query(
          "DELETE FROM NotificationTeacher WHERE Notification_ID = ?",
          req.params.id,
          (err) => {
            if (err)
              res.status(400).send({ success: false, data: err.sqlMessage });
