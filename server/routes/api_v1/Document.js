...
// GET ALL
router.get("/", (req, res) => {
  database.query("SELECT * FROM document AS D ", (err, rows) => {
...
router.post("/", (req, res) => {
  ...
  database.query(
    "SELECT * FROM document AS D " + `WHERE ${whereStr}`,
    (err, rows) => {
      if (err) responseByStatus(res, err, 400, rows);
      else {
...
router.get("/:id", (req, res) => {
  ...
  database.query(
    "SELECT * FROM document AS D " + "WHERE Document_ID = ?",
    [reqParamStr.id],
    (err, rows) => {
      if (err) responseByStatus(res, err, 400, rows);
      else {
...
router.post("/create", (req, res) => {
  var reqBodyStr = req.body;
  database.query("INSERT INTO document SET ?", reqBodyStr, (err, rows) => {
    if (err) responseByStatus(res, err, 400, rows);
    else responseByStatus(res, err, 200, rows);
  });
});
