...
router.get("/", (req, res) => {
  ...
  if (Object.entries(data).length == 0) {
    database.query("SELECT * FROM RequestStatus", (err, rows) => {
    ...
    database.query(
      "SELECT * FROM RequestStatus " + `WHERE ${condition}`,
      (err, rows) => {
        if (err) res.status(400).send({ success: false, data: err.sqlMessage });
        else {
          if (rows.length == 0)
...
router.get("/:id", (req, res) => {
  database.query(
    "SELECT * FROM RequestStatus " + "WHERE RequestStatus_ID = ?",
    [req.params.id],
    (err, rows) => {
      if (err) res.status(400).send({ success: false, data: err.sqlMessage });
      else {
...
router.post("/", (req, res) => {
  const data = {
    RequestStatus_Name: req.body.RequestStatus_Name
  };
  database.query("INSERT INTO RequestStatus SET ?", data, (err) => {
    if (err) res.status(400).send({ success: false, data: err.sqlMessage });
  ...
  database.query(
    ...
    (err, rows) => {
      if (err) res.status(400).send({ success: false, data: err.sqlMessage });
        ...
        database.query(
          "UPDATE RequestStatus SET ? WHERE RequestStatus_ID = ?",
  ...
  database.query(
    ...
    (err, rows) => {
      if (err) res.status(400).send({ success: false, data: err.sqlMessage });
        ...
        database.query(
          "DELETE FROM RequestStatus WHERE RequestStatus_ID = ?",
          req.params.id,
          (err) => {
            if (err)
              res.status(400).send({ success: false, data: err.sqlMessage });
