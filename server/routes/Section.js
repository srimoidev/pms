...
router.get("/", (req, res) => {
  ...
  if (Object.entries(data).length == 0) {
    database.query("SELECT * FROM Section", (err, rows) => {
    ...
    database.query(
      "SELECT * FROM Section " + `WHERE ${condition}`,
      (err, rows) => {
        if (err) res.status(400).send({ success: false, data: err.sqlMessage });
        else {
          if (rows.length == 0)
...
router.post("/", (req, res) => {
  const data = {
    Section_Year: req.body.Section_Year,
    Section_Term: req.body.Section_Term,
  };
  database.query("INSERT INTO Section SET ?", data, (err) => {
    if (err) res.status(400).send({ success: false, data: err.sqlMessage });
